...
#define DO_TEST_REPEATED(lc_member_name, cast, \
  ...
  uint8_t *data; \
  unsigned i; \
  mess.n_##lc_member_name = ARRAY_SIZE (static_array); \
  mess.lc_member_name = cast static_array; \
  mess2 = test_compare_pack_methods ((ProtobufCMessage*)(&mess), &len, &data); \
  assert(mess2->n_##lc_member_name == ARRAY_SIZE (static_array)); \
  for (i = 0; i < ARRAY_SIZE (static_array); i++) \
